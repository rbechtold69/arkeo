...
func TestPaidTierRejectsDifferentSignerAndOverflow(t *testing.T) {
	...
	auth.Spender = c.Client
	auth.Nonce = math.MaxInt64
	code, err = p.paidTier(auth, "")
	require.Error(t, err)
...
func TestConcurrentReplayCanOnlyBeAcceptedOnce(t *testing.T) {
	...
	for i := 0; i < 32; i++ {
		...
		go func() {
			defer wg.Done()
			if code, err := p.paidTier(auth, ""); err == nil && code == http.StatusOK {
				accepted.Add(1)
			}
		}()
	}
...
func TestDelayedSettlementCannotOverwriteNewClaim(t *testing.T) {
	...
	require.NoError(t, err)
	defer store.Close()
	first := NewClaim(9, nil, 1, "first")
	require.NoError(t, store.Accept(first))
	require.NoError(t, store.Accept(NewClaim(9, nil, 2, "second")))
	marked, err := store.MarkClaimed(9, 1)
	require.NoError(t, err)
	...
	marked, err = store.MarkClaimed(9, 2)
	require.NoError(t, err)
	require.True(t, marked)
	require.Error(t, store.Accept(NewClaim(9, nil, 2, "replayed")))
	require.Error(t, store.Batch([]Claim{NewClaim(9, nil, 4, "new"), NewClaim(9, nil, 3, "older")}))
	got, err = store.Get("9")
	require.NoError(t, err)
	require.EqualValues(t, 2, got.Nonce)
}
...
func TestSharedNonceStoreReservesDistinctDurableValues(t *testing.T) {
	...
	require.Error(t, store.Set(42, 2))
	require.NoError(t, store.Close())
	reopened, err := NewNonceStore(dir)
	require.NoError(t, err)
	defer reopened.Close()
	n, err := reopened.ReserveAfter(42, 0)
	require.NoError(t, err)
	require.EqualValues(t, 33, n)
}
...
func TestRegistryFetchHasBoundedTimeAndRejectsDuplicateServices(t *testing.T) {
	...
	duplicate := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		_, _ = w.Write([]byte(`{"services":[{"name":"btc-mainnet-fullnode","service_id":10},{"name":"btc-mainnet-fullnode","service_id":11}]}`))
	}))
...
func TestContractLookupRejectsMalformedAndMismatchedAccounting(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		value any
		valid bool
	}{{"id", "42", true}, {"id", "43", false}, {"nonce", "-1", false}, {"deposit", "bad", false}, {"paid", "101", false}, {"duration", fmt.Sprint(int64(math.MaxInt64)), false}, {"type", "UNRECOGNIZED", false}} {
		t.Run(tc.field+fmt.Sprint(tc.value), func(t *testing.T) {
			...
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				_ = json.NewEncoder(w).Encode(map[string]any{"contract": data})
			}))
			defer server.Close()
			store := NewMemStore(server.URL, nil, log.NewNopLogger())
			c, err := store.fetchContract("42")
			if tc.valid {
				require.NoError(t, err)
				require.EqualValues(t, 20, c.SettlementDuration)
			} else {
				require.Error(t, err)
			}
		})
...
func TestHeightReadWriteIsRaceSafe(t *testing.T) {
	store := NewMemStore("", nil, log.NewNopLogger())
	var wg sync.WaitGroup
	for i := 0; i < 8; i++ {
		wg.Add(1)
		go func() {
